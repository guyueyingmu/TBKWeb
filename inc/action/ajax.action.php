...
class ajax extends app{
	...
	function like(){
		...
		if(!$rs ||  !$rs[id]){
					...
					$fd = table($table);
						...
						if($where) {
								DB::query("UPDATE ".DB::table($table)." SET `$fd_name` = `$fd_name` +1  WHERE $where ");
					...
					if(isset($fd['like_count']) || isset($fd['like'])){
						...
						if($where){
							DB::query("UPDATE ".DB::table($table)." SET `$fd_name` = `$fd_name` -1  WHERE $where AND `$fd_name` > 0 ");
	...
	function get_share_text(){
		...
		if(!$_GET['num_iid']) json('商品id不存在');
		$num_iid = get_goods_id($_GET['num_iid']);
		$goods = D(array('and'=>"num_iid = '$num_iid'",'limit'=>1));
		if(!$goods['aid'] ) json('抱歉,当前商品已下架或未审核暂时无法查看');
		$text = $this->make_text($goods);
		json(array('msg' => '复制成功', 'status' => 'success','data'=>$text));
	}

	private function make_text($goods){
		global $_G;
		$rs = array();
		$rs[] = '<img src="'.$goods['picurl'].'_200x200.jpg'.'" />';
		$rs[] = $goods['title'];
		
		if($goods['juan_url']){
			$rs[] = '原价'.$goods['yh_price'].'元' .($goods['sum']>999 ? '已售'.$goods['sum'].'件':'');
			$rs[] = '券后【'.($goods['yh_price'] - $goods['juan_price']).'元】包邮秒杀';
			//$rs[] = '<p class="copy_url">领券及下单链接'.$goods['juan_url'].'</p>';
			$rs[] = '<p class="copy_url">领券及下单链接'.$_G['siteurl'].$goods['id_url'].'&focus=1</p>';
		}else{
			$rs[] = '原价'.$goods['price'].'元' .($goods['sum']>999 ? '已售'.$goods['sum'].'件':'');
			$rs[] = '现价【'.($goods['yh_price']).'元】包邮秒杀';
			//$rs[] = '<p class="copy_url">下单链接'.$goods['url'].'</p>';
			$rs[] = '<p class="copy_url">下单链接'.$_G['siteurl'].$goods['id_url'].'&focus=1</p>';
		}
		if($goods['tkl']){
			$rs[] = '复制淘口令到手机淘宝中打开'.$goods['tkl'];
		}else{
			$data = get_tkl($goods['aid']);
			if($data['status'] =='success'){
				$rs[] = '复制淘口令到手机淘宝中打开'.$data['data'];
			}
		}

		if($goods['ly'])$rs[] = $goods['ly'];
