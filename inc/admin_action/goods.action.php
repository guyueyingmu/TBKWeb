...
class goods extends app{
		...
		function goods_check(){
			...
			$check_list = unserialize($_G['setting']['goods_check']);
			
			$this->add(array('check_list'=>$check_list));
			parent::seo_setting();
		}
...
function quan_import(){
			...
			if($_GET['onsubmit'] ){
							...
							foreach ($data as $k => $v) {
								...
								preg_match("/满([\d\.]{1,6})元减([\d\.]{1,6})元/is",$v[17],$tmp);
								if($tmp && $tmp[2]){
									$arr['juan_price']    = $tmp[2];
								}else if(!$tmp && strpos($v[15],'元无条件券') !== false){
									$arr['juan_price']    = str_replace('元无条件券','',$v[15]);
								...
								$arr['start_time']       = dmktime($v[18]);
								$arr['end_time']       = dmktime($v[19]);

								$arr['juan_url']    = $v[21];
								$pid = sub_str($v[21],'&pid=','&af=1');
								$arr['juan_url']  = 'https://uland.taobao.com/coupon/edetail?activityId='.$v[14];
								$arr['juan_url'] .='&itemId='.$v[0].'&pid='.$pid.'&src=kfz_utao&sid='.$v[11];

								//$arr['juan_url']  = $v[21].'&src=kfz_utao';
	...
	function tkl(){
		...
		function tkl_create(){
				...
				foreach($rs as $k=>$v){
					...
					$url = $v['juan_url'];
					if(!$url) $url = $v['url'];
					 $tkl = $mama->tkl($url,$share_text,$v['picurl']);
